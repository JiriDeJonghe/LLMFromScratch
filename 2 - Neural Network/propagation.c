...
void print_network_gradients(NetworkGradients* network_gradients) {
    for (size_t l = 0; l < network_gradients->num_layers; l++) {
        LayerGradients* layer_gradients = network_gradients->layer_gradients[l];
        printf("\nLayer %zu\n", l);
        for (size_t n = 0; n < layer_gradients->num_neurons; n++) {
            NeuronGradients* neuron_gradients = layer_gradients->neuron_gradients[n];
            printf("Neuron %zu\n", n);
            for (size_t w = 0; w < neuron_gradients->num_weights; w++) {
                printf("Gradient %zu: %f\n", w, neuron_gradients->weight_gradients[w]);
            }
        }
    }
}
