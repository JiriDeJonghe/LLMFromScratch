...
/**
 * @brief Calculates a forward pass of the neural network. The final results can
 * be found by inspecting the final layers
 *
 ...
 * @param output_activation The activation to apply at the end of the neural
 * network
 */
void forward_propagate(NeuralNetwork *network, const float *input,
                       LayerActivationFunc *output_activation) {
  ...
  //  Iterate over the layers from Left to Right (with Left being the input, and
  //  Right being the output layer) Determine the output of each neuron by
  //  getting the weighted sum (with bias) and applying the activation function
  for (size_t layer_id = 1; layer_id < network->num_layers; layer_id++) {
    ...
    for (size_t neuron_id = 0; neuron_id < current_layer->num_neurons;
      ...
      for (size_t prev_neuron_id = 0;
           prev_neuron_id < previous_layer->num_neurons; prev_neuron_id++) {
      ...
      if (layer_id != network->num_layers - 1) {
        ...
        current_neuron->output =
            weighted_sum; // Store raw value for softmax in last layer to get
                          // probabilities
      }
    }

    if (layer_id == network->num_layers - 1) {
      output_activation(current_layer);
    }
  }
}

/**
 * @brief Calculates a backward pass of the neural network. The weights and
 * biases will be updated accordingly.
 *
 ...
 */
void backward_propagate(NeuralNetwork *network, NetworkGradients *gradients,
                        const float *input, const float *result,
                        const float learning_rate) {
  if (network == NULL || gradients == NULL || input == NULL || result == NULL ||
  ...
  for (size_t i = 0; i < network->num_layers; i++) {
    ...
    for (size_t j = 0; j < layer->num_neurons; j++) {
      ...
      for (size_t k = 0; k < neuron->num_inputs; k++) {
        gradients->layer_gradients[i]
            ->neuron_gradients[j]
            ->weight_gradients[k] = 0.0f;
      }
  ...
  for (size_t i = 0; i < output_layer->num_neurons; i++) {
    ...
    // With C the cost function, R the activation function for the output layer
    // and Z_0 the weighted sums of the nodes of the output layer For cross
    // entropy after soft max this is: y_pred - y_true
    float error = output_layer->neurons[i]->output - result[i];

    // After that we calculate the derivative of the Cost with respect to the
    // edges incoming into the nodes of the output layer. The gradient for each
    // of these edges is defined as w_ij = error * o_j
    for (size_t j = 0; j < output_layer->neurons[i]->num_inputs; j++) {
      gradients->layer_gradients[output_layer_idx]
          ->neuron_gradients[i]
          ->weight_gradients[j] = error * prev_layer->neurons[j]->output;
    }
    gradients->layer_gradients[output_layer_idx]->neuron_gradients[i]->error =
        error;
  }

  // Now that we have defined the error and the gradients for the last layer, we
  // propagate it through the network Calculate the gradients of the hidden
  // layers: E_h = E_(l+1) * W_(l+1) * R'(Z_h)
  // The input layer should not have any gradients associated so we limit to
  // layers >= 1
  for (int i = (int)network->num_layers - 2; i >= 1; i--) {
    Layer *layer = network->layers[i];
    ...
    for (size_t j = 0; j < layer->num_neurons; j++) {
      ...
      for (size_t next_j = 0; next_j < next_layer->num_neurons; next_j++) {
        float next_error =
            next_layer_gradients->neuron_gradients[next_j]->error;
      ...
      if (i > 0) {
        ...
        for (size_t w = 0; w < layer->neurons[j]->num_inputs; w++) {
          layer_gradients->neuron_gradients[j]->weight_gradients[w] =
              error * prev_layer->neurons[w]->output;
        }
      } else {
        for (size_t w = 0; w < layer->neurons[j]->num_inputs; w++) {
          layer_gradients->neuron_gradients[j]->weight_gradients[w] =
